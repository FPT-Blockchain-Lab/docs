...
<p align="center">
  <img width="500" height="500" src="./images/Management.png">
</p>
...
- The API module:
    - `Standard Factory`: 
        - Manage a collection of `Standard LC` contracts
        - Record the address of `AMC` -> each of `Standard LC` contracts can easily update/access the latest settings
    - `UPAS Factory`: 
        - Manage a collection of `UPAS LC` contracts
        - Record the address of `AMC` -> each of `UPAS LC` contracts can easily update/access the latest settings
    - `AmendRequest`
    ...
    - `RouterService`
        ...
        - Record the address of `AMC` -> quickly update/link to new address of other contracts in a cross-contracts call
- The DBMS: a collection of `Standard LC` and `UPAS LC` contracts
    - Each of `Standard LC` contract manages proof of approvals relating to one standard LC document
    - Each of `UPAS LC` contract manages proof of approvals relating to one UPAS LC document
    - `Standard LC` and `UPAS LC` contracts are owned by `Standard Factory` and `UPAS Factory` accordingly. Thus, accessing higher management level must go through these two factories

<p align="center">
  <img width="600" height="400" src="./images/Hierarchy_Management.png">
</p>
